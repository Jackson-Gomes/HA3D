...
function ensureState(panel) {
  ...
  if (typeof panel._ha3dCinematicPrepActive !== "boolean") {
    panel._ha3dCinematicPrepActive = false;
  }
  if (!Array.isArray(panel._ha3dCinematicPrepPending)) {
    panel._ha3dCinematicPrepPending = [];
  }
...
function ensureStyle(panel) {
  ...
  let style = panel.shadowRoot.querySelector("#ha3dXrayIdleStyleV4");
  if (!style) {
    style = document.createElement("style");
    style.id = "ha3dXrayIdleStyleV4";
...
function restoreIdleControls(panel) {
...
function checkIdle(panel) {
  ensureState(panel);
  if (panel._ha3dIdleActive || panel._ha3dIdleExitSequence || panel._ha3dCinematicPrepActive) return;
...
async function prepareIdleForCinematic(panel) {
  ensureState(panel);
  if (!panel._ha3dIdleActive) return;
  if (panel._ha3dCinematicPrepPromise) return panel._ha3dCinematicPrepPromise;

  panel._ha3dCinematicPrepPromise = (async () => {
    panel._ha3dCinematicPrepActive = true;
    panel._ha3dResumeIdleAfterCinematic = true;

    // Stop only the idle orbit. Keep the X-Ray material and hidden UI while the
    // camera returns to the deterministic Superior view.
    clearIdleAnimation(panel);
    panel._ha3dIdleActive = false;
    panel._ha3dIdleOrbit = null;
    panel._cameraAnimating = false;
    ...
    // Only after reaching Superior do we reveal the normal GLB. The existing
    // Cinematic focus animation will start from this exact camera position.
    panel.shadowRoot?.querySelector("#root")?.classList.remove("ha3d-idle-xray");
    applyXray(panel, false);
    panel._ha3dIdleControlsState = null;
    panel._ha3dCinematicPrepActive = false;
  })().finally(() => {
    panel._ha3dCinematicPrepPromise = null;
  });

  return panel._ha3dCinematicPrepPromise;
}

function queueCinematicAfterTop(panel, entities, originalEnqueue) {
  ensureState(panel);
  if (Array.isArray(entities)) panel._ha3dCinematicPrepPending.push(...entities);

  if (panel._ha3dCinematicPrepDispatching) return;
  panel._ha3dCinematicPrepDispatching = true;

  prepareIdleForCinematic(panel)
    .catch((error) => console.error("[HA3D] cinematic top preparation failed", error))
    .finally(() => {
      const pending = [...new Set(panel._ha3dCinematicPrepPending)];
      panel._ha3dCinematicPrepPending.length = 0;
      panel._ha3dCinematicPrepDispatching = false;
      if (pending.length) originalEnqueue.call(panel, pending);
    });
}

function installInteractionHooks(panel) {
  const root = panel.shadowRoot?.querySelector("#root");
  if (!root || root.__ha3dIdleXrayHooksV4) return;
  root.__ha3dIdleXrayHooksV4 = true;
...
if (!proto.__ha3dXrayIdleV4) {
  proto.__ha3dXrayIdleV4 = true;
  ...
  proto._enqueueCinematicBatch = function (...args) {
    ...
    if (this._ha3dIdleActive || this._ha3dCinematicPrepActive || this._ha3dCinematicPrepPromise) {
      queueCinematicAfterTop(this, entities, originalEnqueue);
      return;
    }
  ...
  proto._runCinematicFocus = function (...args) {
    if (this._ha3dIdleActive) {
      const entities = args[0];
      return prepareIdleForCinematic(this).then(() => originalRun?.call(this, entities));
    }
